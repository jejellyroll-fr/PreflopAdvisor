...
def looks_like_cards(text: str) -> bool:
    ...
    Reading a class as cards would convert it into another class entirely -- ``KA23`` becomes
    the hold'em hand ``K2o`` -- so the check is that every second character is a suit and
    every other one a rank.
    """
...
def hand_key_of(text: object) -> str:
    """The canonical key of a hand, whichever way a source spelled it.

    ``AhKs4h3s`` and ``(3K)(4A)`` are one key, as they are to every reader. A source that
    already holds keys is left alone -- including the ones that look like cards to a
    converter -- and a hand nothing can read is kept as written, because a hand that cannot be
    classified is still the hand the row was about.
    """
    hand = str(text).strip()
    if not hand:
        return hand
