...
5. Export your Monker Preflop trees:
   ...
   - Or skip the export: a MonkerSolver save (=.mkr=) can be imported directly.
     In the import wizard, choose *Choose a save...* and pick the file; the wizard
     reads the game, seats, depth and ante from the save itself. Saves written by
     MonkerSolver 2.1.9 (for storage or for further calculation) and by 2.3.10-beta
     (for storage) are read; anything else is refused with the reason, and the export
     above still works for it. See =docs/native-import.md=.
6. Configure the Tool:
    - Most of the configuration is done in the application, in the
      *Configuration* tab (next to Advisor and Trainer): sims, sizings, seats,
      reading and display settings. Changes are saved to your own override file
      under the application config location and never touch the shipped
      =config.ini=, so its comments and sample trees survive.
    - If you prefer editing text, open =preflop_advisor/config.ini= in a text
      editor (e.g. Notepad, nano, vim). The sections below are the ones worth
      knowing:
      - [TreeInfos]: make an entry for each tree:
        ...
        + A MonkerSolver save is an entry whose path names the =.mkr= file itself:
          =Table13=4,5,PLO,ranges/ggpoker-aof-plo.mkr,AoF= with =Table13.kind=mkr=.
 ...
 7. (Optional) Adjust Font Sizes for High-Resolution Monitors:
    ...
    - TreeSelector Font: In the Configuration tab (Display) or =config.ini= under [TreeSelector], update =FontSize=.
    - PositionSelector Font: In the Configuration tab (Seats) or =config.ini= under [PositionSelector], update =FontSize=.
...
The Configuration tab's *Sizings* panel does this for you: it lists the action codes
your sims actually contain and flags the ones the tool cannot yet decode, so you only
declare the sizings your trees use.
...
#+BEGIN_SRC sh
uv sync --extra dev
uv run pytest              # 536 tests, coverage gate at 90%
