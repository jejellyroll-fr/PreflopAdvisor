...
whenever the range files change -- the number of files and the newest modification time are
recorded in it and checked on open. The range files are never deleted: they are what a
rebuild reads.
...
#+BEGIN_SRC sh
uv sync --extra dev
uv run pytest              # 310 tests, coverage gate at 90%
...
Select the desired hand. Clicking a selected card unselects it. Once the hand is full, clicking another card replaces the oldest one rather than clearing the whole selection.
