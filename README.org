...
declares, and a tree whose sizings cannot be read is drawn without them rather than with
invented ones.
...
(=12sb=), so a code such as =15= may be one, or an index, or a house convention. A tree
whose sizings cannot be read is drawn without its pot and without its stacks rather than
with invented ones. You can declare what yours mean:
...
- *The table.* Seats in acting order with you brought to the near side, what each one did
  ...
  more than the pot that call makes. A tree using a sizing whose code has no published
  meaning is drawn without a pot and without stacks, rather than with numbers that might
  be wrong. Two things have to be readable for the table to carry numbers, and both can be
